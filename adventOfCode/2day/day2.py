...
def restore_gravity(nums):
	restored_program = nums

	starting_index = 0
	while True:
		if restored_program[starting_index] == 99:
			return restored_program
		
		# adding 
		if restored_program[starting_index] == 1:
			restored_program[restored_program[starting_index+3]] = restored_program[restored_program[starting_index+1]] + restored_program[restored_program[starting_index+2]]
			starting_index += 4
			continue

		# multipling
		if restored_program[starting_index] == 2:
			restored_program[restored_program[starting_index+3]] = restored_program[restored_program[starting_index+1]] * restored_program[restored_program[starting_index+2]]
			starting_index += 4
			continue
...
def main():
	...
	### PART ONE ###
	print(len(ex_input))
	result = restore_gravity(ex_input)
	print(result)
